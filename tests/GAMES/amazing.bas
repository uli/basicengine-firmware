...
240 R=R+1
250 IF WW(R,S)=0 THEN 210
260 IF R-1=0 THEN 530
265 IF WW(R-1,S)<>0 THEN 530
270 IF S-1=0 THEN 390
280 IF WW(R,S-1)<>0 THEN 390
290 IF R=H THEN 330
300 IF WW(R+1,S)<>0 THEN 330
310 X=INT(RND(1)*3+1)
320 ON X GOTO 790,820,860
330 IF S<>V THEN 340
334 IF Z=1 THEN 370
338 Q=1:GOTO 350
340 IF WW(R,S+1)<>0 THEN 370
350 X=INT(RND(1)*3+1)
360 ON X GOTO 790,820,910
370 X=INT(RND(1)*2+1)
380 ON X GOTO 790,820
390 IF R=H THEN 470
400 IF WW(R+1,S)<>0 THEN 470
405 IF S<>V THEN 420
410 IF Z=1 THEN 450
415 Q=1:GOTO 430
420 IF WW(R,S+1)<>0 THEN 450
430 X=INT(RND(1)*3+1)
...
480 IF Z=1 THEN 520
485 Q=1:GOTO 500
490 IF WW(R,S+1)<>0 THEN 520
500 X=INT(RND(1)*2+1)
510 ON X GOTO 790,910
520 GOTO 790
530 IF S-1=0 THEN 670
540 IF WW(R,S-1)<>0 THEN 670
545 IF R=H THEN 610
547 IF WW(R+1,S)<>0 THEN 610
550 IF S<>V THEN 560
552 IF Z=1 THEN 590
554 Q=1:GOTO 570
560 IF WW(R,S+1)<>0 THEN 590
570 X=INT(RND(1)*3+1)
...
620 IF Z=1 THEN 660
625 Q=1:GOTO 640
630 IF WW(R,S+1)<>0 THEN 660
640 X=INT(RND(1)*2+1)
650 ON X GOTO 820,910
660 GOTO 820
670 IF R=H THEN 740
680 IF WW(R+1,S)<>0 THEN 740
685 IF S<>V THEN 700
690 IF Z=1 THEN 730
695 Q=1:GOTO 830
700 IF WW(R,S+1)<>0 THEN 730
710 X=INT(RND(1)*2+1)
...
905 GOTO 530
910 IF Q=1 THEN 960
920 WW(R,S+1)=C:C=C+1:IF VV(R,S)=0 THEN 940
