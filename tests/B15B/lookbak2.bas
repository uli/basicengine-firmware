...
222 REM  For user-specified TN, we now calculate HN, but since O_RDX depends
...
234 REM  O_RDX is Omega(Radiation), which is radiation pressure, which was a
236 REM  major contributor to expansion when the universe was very young.
240 O_RDX=.4165/(HN*HN) : OMC=1-OMG-LAM-O_RDX : Z=5 : AZ=1/(1+Z) : AA=0
...
260 TRS=TRS+1/SQR(OMC+(OMG/A)+(O_RDX/(A*A))+(LAM*A*A))
270 TLB=TLB+1/SQR(OMC+(OMG/AA)+(O_RDX/(AA*AA))+(LAM*AA*AA)) : NEXT I
...
430 TRS=TRS+1/SQR(OMC+OMG/A+O_RDX/(A*A)+LAM*A*A) : NEXT I
440 TT=TRS/N*HFAC/HN*AZ : T_OL=T_OL+DELTA
450 IF TT/TTT>1-T_OL AND TT/TTT<1+T_OL GOTO 480
...
490 A=AZ+(1-AZ)*(I-.5)/N : ADOT=SQR(OMC+(OMG/A)+(O_RDX/(A*A))+(LAM*A*A))
...
600 A=AZ*(I-.5)/N : AGE=AGE+1/SQR(OMC+OMG/A+O_RDX/(A*A)+LAM*A*A) : NEXT I
610 TT=HFAC/HN*AZ*AGE/N : TV=TN-TT : DMOD=5*.4343*LOG(DL*1E+09/(10*PARSEC))
620 IF TT<300000 GOTO 380
630 HT=HN*SQR(OMG*(1+Z)^3+(1-OMG-LAM-O_RDX)*(1+Z)^2+LAM+O_RDX*(1+Z)^4)
