...
helptext_en.json: scripts/help.py $(shell ls basic*.cpp) $(shell ls ../po/helptext_*.po)
	for i in en de fr es ja ; do cat *.cpp|sed -f scripts/bdoc_1.sed  |python3 scripts/help.py helptext_$${i}.json $${i} ; done

autogen_clean:
	rm -f *funtbl.h kwtbl.h kwenum.h epigrams.h version.h helptext_??.json msgs_??.h
