...
EBAPI int eb_add_command(const char *name, const token_t *syntax, eb_command_handler_t handler) {
...
EBAPI int eb_add_numfun(const char *name, const token_t *syntax, eb_numfun_handler_t handler) {
...
EBAPI int eb_add_strfun(const char *name, const token_t *syntax, eb_strfun_handler_t handler) {
...
EBAPI const token_t *eb_command_syntax(token_t token) {
...
EBAPI const token_t *eb_numfun_syntax(token_t token) {
...
EBAPI const token_t *eb_strfun_syntax(token_t token) {
...
EBAPI const char **eb_kwtbl(void) {
    return &kwtbl[0];
}

EBAPI int eb_kwtbl_size(void) {
    return kwtbl.size();
}

EBAPI void *eb_new_basic_context(void) {
    return (void *) new Basic;
}

EBAPI void eb_delete_basic_context(void *bc) {
    delete (Basic *)bc;
}

EBAPI int eb_exec_basic(void *bc, const char *filename) {
    return ((Basic *)bc)->exec(filename);
}

EBAPI void eb_set_error(int error, const char *expected) {
    err = error;
    err_expected = expected;
}

EBAPI int eb_get_error(void) {
    return err;
}
