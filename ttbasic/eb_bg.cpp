...
EBAPI int eb_bg_set_window(int bg, int win_x, int win_y, int win_w, int win_h) {
  if (check_param(bg, 0, MAX_BG - 1) ||
...
EBAPI int eb_bg_disable(int bg) {
  if (check_param(bg, 0, MAX_BG - 1))
      return -1;
...
EBAPI int eb_bg_set_priority(int bg, int priority) {
  if (check_param(bg, 0, MAX_BG - 1) ||
...
EBAPI int eb_bg_load(int bg, const char *file) {
  if (check_param(bg, 0, MAX_BG - 1))
      return -1;
...
EBAPI int eb_bg_save(int bg, const char *file) {
  if (check_param(bg, 0, MAX_BG - 1))
      return -1;
...
EBAPI int eb_bg_move(int bg, int x, int y) {
  if (check_param(bg, 0, MAX_BG - 1))
    return -1;
...
EBAPI int eb_bg_map_tile(int bg, int from, int to) {
  if (check_param(bg, 0, MAX_BG - 1) ||
...
EBAPI int eb_bg_set_tiles(int bg, int x, int y, const uint8_t *tiles, int count) {
  if (check_param(bg, 0, MAX_BG - 1) ||
...
EBAPI int eb_bg_set_tile(int bg, int x, int y, int t) {
  if (check_param(bg, 0, MAX_BG - 1) ||
      check_param(x, 0, INT_MAX) ||	// XXX: is that really a reasonable limit?
      check_param(y, 0, INT_MAX) ||
      check_param(t, 0, 255))
...
EBAPI int eb_bg_win_width(int bg) {
  if (check_param(bg, 0, MAX_BG - 1))
    return -1;
...
EBAPI int eb_bg_win_height(int bg) {
  if (check_param(bg, 0, MAX_BG - 1))
    return -1;
...
EBAPI int eb_bg_enabled(int bg) {
  if (check_param(bg, 0, MAX_BG - 1))
    return -1;
...
EBAPI int eb_add_bg_layer(eb_layer_painter_t painter, int prio, void *userdata) {
  return vs23.addBgLayer(painter, prio, userdata);
}

EBAPI void eb_remove_bg_layer(int id) {
