...
#define m_pal          vs23_int.pal
        ...
        uint8_t *scr_line = (uint8_t *)screen->pixels + SDL_X_OFF +
                            (int((i + STARTLINE - VIEWPORT_Y) * STRETCH_Y) *
                             screen->pitch) +
                            SDL_Y_OFF;
        ...
        for (int j = 0; j < STRETCH_Y; ++j) {
          memcpy(scr_line + j * screen->pitch,
                 screen->pixels + SDL_X_OFF + SDL_Y_OFF,
                 xoff * screen->format->BytesPerPixel);
          memcpy(scr_line + j * screen->pitch +
                         (xoff + vs23.width()) * screen->format->BytesPerPixel,
                 (uint8_t *)screen->pixels + SDL_X_OFF + SDL_Y_OFF +
                         (xoff + vs23.width()) * screen->format->BytesPerPixel,
                 (SDL_X_SIZE - vs23.width() - xoff) *
      ...
      for (int i = 1; i < int((STARTLINE - VIEWPORT_Y) * STRETCH_Y); ++i) {
        memcpy(p, screen->pixels + SDL_X_OFF + SDL_Y_OFF,
               SDL_X_SIZE * screen->format->BytesPerPixel);
        p += screen->pitch;
      }
      // draw SDL screen border bottom (after end of picture)
      for (int i = (STARTLINE - VIEWPORT_Y + vs23.height()) * STRETCH_Y;
           i < SDL_Y_SIZE; ++i) {
        memcpy((uint8_t *)screen->pixels + SDL_X_OFF + SDL_Y_OFF +
                       i * screen->pitch,
               screen->pixels + SDL_X_OFF + SDL_Y_OFF,
               SDL_X_SIZE * screen->format->BytesPerPixel);
      }
      if (vid_fp) {
        ...
        for (int i = STARTLINE - VIEWPORT_Y + vs23.height(); i < YUV_Y_SIZE;
             ++i) {
          memcpy(&y_screen[i * SDL_X_SIZE], y_screen, SDL_X_SIZE);
          memcpy(&u_screen[i * SDL_X_SIZE], u_screen, SDL_X_SIZE);
          memcpy(&v_screen[i * SDL_X_SIZE], v_screen, SDL_X_SIZE);
...
uint32_t Video::frame() {
  ...
  // demand.
  m_frame = (micros() / vs23_int.line_us - vs23.m_sync_line) /
            vs23_int.line_count;
  return m_frame;
}
