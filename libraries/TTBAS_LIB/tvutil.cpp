...
//
void GROUP(basic_video) tv_clerLine(uint16_t l, int from) {
  int x = win_x + from * f_width;
  int y = win_y + l * f_height;
  int w = win_width - from * f_width;

  // fill top pixels with background
  while (!blockFinished()) {}
#ifdef USE_VS23
  // line at most two chars then duplicate with blitter
  int preset = f_width + ((win_c_width == 1) ? 0 : f_width);
  gfx.drawLine(x, y, x + preset -1, y, bg_color);

  // Apparently source and destination address have to be
  // at least 4 bytes apart. Alignment is not an issue.
  if (win_c_width > 2) {
    int adjust = win_width - win_c_width * f_width;
    int length = win_c_width - ((adjust) ? 1 : 2);
    int target = x + f_width + ((adjust) ? adjust : f_width);

    // special mode: skip disabled
    vs23.MoveBlock(x, y, target, y, f_width, length, 2);
  }
#else
  gfx.drawLine(x, y, x + w -1, y, bg_color);
#endif

  // duplicate top line
  while (w >= 256) {
    vs23.MoveBlock(x, y, x, y +1, 240, f_height -1, 0);
    x += 240;   // we can do 255 but we need
    w -= 240;   // at least 5 for the tail below
  }
  vs23.MoveBlock(x, y, x, y +1, w, f_height -1, 0);
}
