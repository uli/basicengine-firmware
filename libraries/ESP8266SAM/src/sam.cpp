...
void ESP8266SAM::InsertBreath()
{
  ...
  while (1)
  {
    ...
    if (mem55 < 232)
    {
      ...
      {
	A = pgm_read_byte(&flags2[index])&1;
	if (A != 0)
	{
	  X++;
	  mem55 = 0;
...
void ESP8266SAM::CopyStress()
{
  ...
  while (1)
  {
    ...
    if ((pgm_read_byte(&flags[Y]) & 64) == 0) {pos++; continue; }
    ...
    if ((pgm_read_byte(&flags[Y]) & 128) == 0) {pos++; continue; }
...
void ESP8266SAM::Code41240()
{
  ...
  while (phonemeindex[pos] != 255)
  {
    ...
    if ((pgm_read_byte(&flags[index])&1) == 0)
    {
    ...
    if (A != 255)
    {
      if ((pgm_read_byte(&flags[A]) & 8) != 0) {pos++; continue; }
...
// Check for DIPHTONG
    if ((pgm_read_byte(&flags[A]) & 16) == 0) goto pos41457;

// Not a diphthong. Get the stress
    mem58 = stress[pos];

// End in IY sound?
    A = pgm_read_byte(&flags[Y]) & 32;
...
// Insert at WX or YX following, copying the stress
    ...
    if (A == 20) dbg_sam("RULE: insert WX following diphtong NOT ending in IY sound\n");
    if (A == 21) dbg_sam("RULE: insert YX following diphtong ending in IY sound\n");
...
// Insert a glottal stop and move forward
	      dbg_sam("RULE: Insert glottal stop between two stressed vowels with space between them\n");
	      // 31 = 'Q'
	      Insert(X, 31, mem59, 0);
	      pos++;
	      continue;
...
// If prior phoneme does not have VOWEL flag set, move to next phoneme
      if ((pgm_read_byte(&flags[phonemeindex[pos-1]]) & 128) == 0) {pos++; continue; }
// Prior phoneme has VOWEL flag set, so change L to LX and move to next phoneme
      dbg_sam("RULE: <VOWEL> L -> <VOWEL> LX\n");
...
// VOWELS AND DIPHTONGS ENDING WITH IY SOUND flag set?
	A = pgm_read_byte(&flags[Y]) & 32;
	if (A == 0) dbg_sam("RULE: K <VOWEL OR DIPHTONG NOT ENDING WITH IY> -> KX <VOWEL OR DIPHTONG NOT ENDING WITH IY>\n");
// Replace with KX
...
// If diphtong ending with YX, move continue processing next phoneme
      if ((pgm_read_byte(&flags[index]) & 32) != 0) {pos++; continue; }
// replace G with GX and continue processing next phoneme
      dbg_sam("RULE: G <VOWEL OR DIPHTONG NOT ENDING WITH IY> -> GX <VOWEL OR DIPHTONG NOT ENDING WITH IY>\n");
...
// ALVEOLAR flag set?
      Y = phonemeindex[X-1];
      A = pgm_read_byte(&flags2[Y]) & 4;
// If not set, continue processing next phoneme
      if (A == 0) {pos++; continue; }
      dbg_sam("RULE: <ALVEOLAR> UW -> <ALVEOLAR> UX\n");
      phonemeindex[X] = 16;
      pos++;
      continue;
    }
...
// If prior phoneme is not a vowel, continue processing phonemes
    if ((pgm_read_byte(&flags[phonemeindex[X-1]]) & 128) == 0) {pos++; continue; }
...
// Is next phoneme a vowel or ER?
	A = pgm_read_byte(&flags[A]) & 128;
      if (A != 0) dbg_sam("RULE: Soften T or D following vowel or ER and preceding a pause -> DX\n");
...
void ESP8266SAM::AdjustLengths()
{
  ...
  while (1)
  {
    ...
    // not punctuation?
    if ((pgm_read_byte(&flags2[index]) & 1) == 0)
    {
      // skip
      X++;
      continue;
...
pos48644:
    ...
    if (index != 255)             //inserted to prevent access overrun
      if ((pgm_read_byte(&flags[index]) & 128) == 0) goto pos48644;          // if not a vowel, continue looping
	...
	if (((pgm_read_byte(&flags2[index]) & 32) == 0) || ((pgm_read_byte(&flags[index]) & 4) != 0))                    //nochmal �berpr�fen
	{
	  //A = pgm_read_byte(&flags[Y]) & 4;
	  ...
	  A = (A >> 1) + A + 1;
	  dbg_sam("RULE: Lengthen <FRICATIVE> or <VOICED> between <VOWEL> and <PUNCTUATION> by 1.5\n");
	  dbg_sam("PRE\n");
	  dbg_sam("phoneme %d (%c%c) length %d\n", X, signInputTable1[phonemeindex[X]], signInputTable2[phonemeindex[X]], phonemeLength[X]);
  ...
  while (1)
  {
    ...
    // vowel?
    A = pgm_read_byte(&flags[index]) & 128;
    if (A != 0)
    {
      // get next phoneme
      X++;
	...
	if ((index == 18) || (index == 19))                          // 'RX' & 'LX'
	{
	  ...
	  if (index < sizeof(flags) && (pgm_read_byte(&flags[index]) & 64) != 0) {
	    ...
	    dbg_sam("RULE: <VOWEL> <RX | LX> <CONSONANT> - decrease length by 1\n");
	    dbg_sam("PRE\n");
	...
	dbg_sam("RULE: <VOWEL> <UNVOICED PLOSIVE> - decrease vowel by 1/8th\n");
	dbg_sam("PRE\n");
	dbg_sam("phoneme %d (%c%c) length %d\n", X, signInputTable1[phonemeindex[X]], signInputTable2[phonemeindex[X]], phonemeLength[X]);
	...
	continue;
      ...
      dbg_sam("RULE: <VOWEL> <VOICED CONSONANT> - increase vowel by 1/2 + 1\n");
      dbg_sam("PRE\n");
...
//pos48821:
    ...
    // nasal?
    if ((pgm_read_byte(&flags2[index]) & 8) != 0)
    {
	...
	A = pgm_read_byte(&flags[index]) & 2;         // check for stop consonant
      ...
      {
	dbg_sam("RULE: <NASAL> <STOP CONSONANT> - set nasal = 5, consonant = 6\n");
	dbg_sam("POST\n");
	dbg_sam("phoneme %d (%c%c) length %d\n", X, signInputTable1[phonemeindex[X]], signInputTable2[phonemeindex[X]], phonemeLength[X]);
	dbg_sam("phoneme %d (%c%c) length %d\n", X-1, signInputTable1[phonemeindex[X-1]], signInputTable2[phonemeindex[X-1]], phonemeLength[X-1]);
	...
	if ((65 & 2) == 0) {loopIndex++; continue; }
      } else if ((pgm_read_byte(&flags[index]) & 2) == 0) {
	...
	continue;
      ...
      dbg_sam("RULE: <UNVOICED STOP CONSONANT> {optional silence} <STOP CONSONANT> - shorten both to 1/2 + 1\n");
      dbg_sam("PRE\n");
      dbg_sam("phoneme %d (%c%c) length %d\n", X, signInputTable1[phonemeindex[X]], signInputTable2[phonemeindex[X]], phonemeLength[X]);
      dbg_sam("phoneme %d (%c%c) length %d\n", X-1, signInputTable1[phonemeindex[X-1]], signInputTable2[phonemeindex[X-1]], phonemeLength[X-1]);
// X gets overwritten, so hold prior X value for debug statement
    ...
    // liquic consonant?
    if ((pgm_read_byte(&flags2[index]) & 16) != 0)
    {
      // R*, L*, W*, Y*
	...
	dbg_sam("RULE: <LIQUID CONSONANT> <DIPHTONG> - decrease by 2\n");
        dbg_sam("PRE\n");
        dbg_sam("phoneme %d (%c%c) length %d\n", X, signInputTable1[phonemeindex[X]], signInputTable2[phonemeindex[X]], phonemeLength[X]);
