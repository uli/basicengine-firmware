...
static void VIA82XX_card_info(struct mpxplay_audioout_info_s *aui)
{
	...
	sprintf(sout, "VIA : %s soundcard found on port:%4.4lX irq:%d chiprev:%2.2X model:%4.4X",
...
#ifdef __DJGPP__
	card->virtualpagetable = (uint32_t*)(((void*)card->virtualpagetable) + __djgpp_conventional_base);
#endif
