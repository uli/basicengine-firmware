...
- 説明  
 ...
 PRINT文にのみ利用可能です.  
 指定した数値が指定桁数を超える場合は、実際の桁数にて出力します.  
...
- 引数  
 モード: 32ビット長の上位、下位の指定  
   LOW  下位15ビット(14-0)の値の取得   
   HIGH 上位15ビット(15-29)の値の取得   

- 戻り値    
 モード LOW時:  0～32767 (0.1秒単位)  
 モード HIGH時: 0～32767 上位ビット(3276.8秒単位)  

- 説明  
 起動からの経過時間を返します.  
 モード指定が無し、または**LOW**指定の場合は0.1秒単位の経過時間を返します.  
